            ...
            <form action="{{ route('tyro-dashboard.roles.index') }}" method="GET">
                <div class="filters-bar">
                    <div class="search-box">
                        ...
                        <input type="text" name="search" class="form-input" placeholder="Search roles..."
                ...
                <table class="table">
                    ...
                    <tbody>
                        @foreach($roles as $role)
                            <tr>
                                <td>
                                    <a href="{{ route('tyro-dashboard.roles.show', $role->id) }}"
                                        style="display: flex; align-items: center; gap: 0.75rem; text-decoration: none;">
                                        <div
                                            style="width: 36px; height: 36px; border-radius: 0.5rem; background: linear-gradient(135deg, #6366f1 0%, #8b5cf6 100%); display: flex; align-items: center; justify-content: center;">
                                            <svg style="width: 18px; height: 18px; color: white;" viewBox="0 0 24 24" fill="none"
                                ...
                                <td>
                                    <code
                                        style="padding: 0.25rem 0.5rem; background-color: var(--muted); border-radius: 0.25rem; font-size: 0.8125rem;">{{ $role->slug }}</code>
                                </td>
                                <td>
                                ...
                                <td>
                                    <form action="{{ route('tyro-dashboard.roles.toggle', $role->id) }}" method="POST">
                                        @csrf
                                        <button type="submit"
                                            class="badge {{ $role->is_active ? 'badge-success' : 'badge-danger' }}"
                                ...
                                <td>
                                    <div class="action-buttons" style="justify-content: flex-end;">
                                        <a href="{{ route('tyro-dashboard.roles.show', $role->id) }}" class="action-btn"
                                            title="View">
                                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                        ...
                                        <a href="{{ route('tyro-dashboard.roles.edit', $role->id) }}" class="action-btn"
                                            title="Edit">
                                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                        ...
                                        @if(!in_array($role->slug, $protectedRoles))
                                            <form action="{{ route('tyro-dashboard.roles.destroy', $role->id) }}" method="POST"
                                                style="display: inline;">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="action-btn action-btn-danger" title="Delete"
                                                    onclick="return confirm('Are you sure you want to delete this role? All users will be unassigned from this role.')">
                                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
