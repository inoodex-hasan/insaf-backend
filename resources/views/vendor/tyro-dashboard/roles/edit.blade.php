            ...
            <div class="card-body">
                <div class="form-row">
                    ...
                    <div class="form-group" style="display: flex; align-items: center; gap: 0.5rem; margin-top: 1.5rem;">
                        <input type="checkbox" id="is_active" name="is_active" class="form-checkbox" value="1" {{ old('is_active', $role->is_active) ? 'checked' : '' }}>
                        ...
                        <div class="checkbox-list">
                            @foreach($privileges as $privilege)
                                <label class="checkbox-item">
                                    <input type="checkbox" name="privileges[]" value="{{ $privilege->id }}" class="checkbox-input"
                                        {{ in_array($privilege->id, old('privileges', $role->privileges->pluck('id')->toArray())) ? 'checked' : '' }}>
                    ...
                    @else
                        <div class="alert alert-info">
                            ...
                            <div class="alert-content">
                                <p class="alert-message">No privileges available. <a
                                        href="{{ route('tyro-dashboard.privileges.create') }}">Create one</a> first.</p>
                            </div>
                        </div>
                    @endif
                    @error('privileges')
